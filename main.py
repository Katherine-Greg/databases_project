...
db_name_old = os.getenv("POSTGRES_DB")
...
with transaction.atomic():
    for row in data_info:
        location, loc_created = Location.objects.get_or_create(region_name=row[3],
                                                     area_name=row[4],
                                                     territory_name=row[5],
                                                     territory_type=row[7])
        institution, inst_created = Institution.objects.get_or_create(name=row[8], type=row[9])
        student, stu_created = Student.objects.get_or_create(birth_year=row[1],
                                                   sex=row[2],
                                                   type=row[6],
                                                   location=location,
                                                   institution=institution)

        result, res_created = StudentResults.objects.get_or_create(student=student,
                                                         status=row[10],
                                                         ukrball100=row[11],
                                                         ukrball12=row[12])
